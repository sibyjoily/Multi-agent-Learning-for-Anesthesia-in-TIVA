...
from dev2.single_agent_model import DDPGAgent, PPOAgent, DQNAgent, DDQNAgent,DuelingDQNAgent
...
class SingleAgentPolicyBased:
    def __init__(self, model_name='DDPG', dataset_name='general', is_sampling=True, scheme='online',
        ...
        self.action_dim = np.shape(self.actions)[1]

        self.action_space = torch.unique(agent_action_space, dim=0)
        ...
        if self.model_name == 'DDPG':
            self.model = DDPGAgent(state_dim=self.state_dim, action_dim=self.action_dim)
        elif self.model_name == 'PPO':
            self.model = PPOAgent(state_dim=self.state_dim, action_dim=self.action_dim,max_value=self.max_value)
        elif self.model_name == 'DQN':
            self.model = DQNAgent(state_dim=self.state_dim, action_dim=self.action_space_dim, action_space=self.action_space)
        elif self.model_name == 'DDQN':
            self.model = DDQNAgent(state_dim=self.state_dim, action_dim=self.action_space_dim, action_space=self.action_space)
        elif self.model_name == 'DuelingDQN':
            self.model = DuelingDQNAgent(state_dim=self.state_dim, action_dim=self.action_space_dim,
    ...
    def data_type_convert(self,trajectory_predict_data):
        ...
        # trajectory_predict_data['bis'] = list(map(lambda x: x.tolist(), trajectory_predict_data['bis']))
